    ...
    <div className="wrap">
      ...
      <div className="sample-grid">
        {samples.map(s=><article key={s.name} ref={el=>{if(el)refs.current.set(s.name,el)}} className={`project-card project-card--${s.theme}`} role="button" tabIndex={0} aria-haspopup="dialog" aria-label={`Open details for ${s.name}`} onClick={()=>openProject(s.name)} onKeyDown={e=>handleKey(e,s.name)}>
          {motif(s.theme)}
          <div className="project-card-content"><div className="project-card-head"><div><span className="sample-label">Sample build · not client work</span><h3>{s.name}</h3></div><span className="status-chip">{s.theme==='realtime'?'LIVE (simulated)':s.status}</span></div><p>{s.problem}</p>{s.note&&<p className="sample-note">{s.note}</p>}<ul className="project-bullets">{s.does.map(x=><li key={x}>{x}</li>)}</ul><div className="project-tags">{s.tech.map(x=><span key={x}>{x}</span>)}</div><SystemDiagram architecture={architectures[s.name]!} size="preview"/><div className="project-card-foot"><span>REST</span><span className="btn">View details</span></div></div>
        </article>)}
      </div>
