...
export default function ProjectModal({project,architecture,trigger,onClose}:{project:Project;architecture:Arch;trigger:HTMLElement|null;onClose:()=>void}){
  ...
  return <dialog ref={dialogRef} className={`project-dialog project-dialog--${project.theme}`} aria-modal="true" aria-labelledby="project-dialog-title" onClick={e=>{if(e.target===dialogRef.current)close()}}>
    <div ref={panelRef} className="project-modal-panel">
      <div className="project-grab" aria-hidden/>
      <header className="project-modal-head">
        ...
        <div className="modal-head-actions"><span className="status-chip">{project.status??(project.when?'Private repository':'Planned')}</span><button ref={closeRef} className="modal-close" type="button" aria-label={`Close ${project.name} details`} onClick={close}>×</button></div>
      </header>
      <div className="project-modal-body">
        ...
        <section className="modal-section modal-stagger"><div className="modal-section-head"><p className="eyebrow">ARCHITECTURE</p><span>Hover or focus a node</span></div><SystemDiagram architecture={architecture} size="full" interactive/></section>
        ...
        <div className="project-modal-cta"><a href="#contact" onClick={e=>{e.preventDefault();close();window.setTimeout(()=>document.querySelector('#contact')?.scrollIntoView({behavior:reduced?'auto':'smooth'}),320)}}>Want something like this for your product? → Contact</a></div>
      </div>
    </div>
  </dialog>
}
